...
- Inlines dependencies (like `kenat`) while keeping GJS/GI imports external
...
- Supports custom SSH ports via `-p/--port` (e.g. for GNOME Boxes port forwarding)
...
#### `scripts/update-contrib.js` - Contributor Management
...
**Version Management:**
```bash
bun bump 1.4.0    # Bump version to 1.4.0
bun release 1.4.0 # Bump version, commit, and create git tag
```
...
# or with custom port (e.g. GNOME Boxes):
bun setup -- -p 2222 user@localhost
```

**Run generated script in VM:**
```bash
~/dev-ethcal.sh  # Mount host directory via SSHFS and start shell
```

Once mounted, you are in the project directory at `/mnt/host/ethcal` with an interactive shell.
...
2. On the **VM**, ensure the SSHFS mount is active (`~/dev-ethcal.sh`)
